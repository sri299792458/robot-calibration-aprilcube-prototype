...
Open `G1_R2_fit_coupons.3mf` in Bambu Studio for all four coupons on one plate, or load the individual STLs in `print_parts/`. Keep the supplied orientation and 100% scale. The marked flat face goes on the bed; the curved face prints upward.
...
Left/right refer to the robot's own sides. `renders/identification_marks.png` shows the markings.
