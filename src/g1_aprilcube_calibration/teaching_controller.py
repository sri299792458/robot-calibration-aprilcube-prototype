...
class TeachingArmController:
    ...
    def __init__(
        ...
        *,
        ...
        if self.calibration_arm == "left":
            self._opposite_hold_q = self.activation_q14[7:].copy()
        else:
            self._opposite_hold_q = self.activation_q14[:7].copy()
        self._opposite_monitor_q = self._opposite_hold_q.copy()
    ...
    def acquire(self, *, operator_confirmed: bool) -> None:
        ...
        if self.calibration_arm == "left":
            self._opposite_hold_q = measured[7:].copy()
        else:
            self._opposite_hold_q = measured[:7].copy()
        self._opposite_monitor_q = self._opposite_hold_q.copy()
    ...
    def _freeze_current(self, reason: str) -> None:
        ...
        sample = self.transport.observe()
        self._validate_fresh_state(sample, now)
        self._validate_opposite_arm(sample)
    ...
    def tick(self) -> TeachingState:
        ...
        if self.state is TeachingState.ACQUIRING:
            ...
            if self._weight >= 1.0:
                ...
                self._opposite_monitor_q = sample.arm_q(
                    opposite_arm(self.calibration_arm)
                ).copy()
                self._transition(
                    TeachingState.GUIDE,
    ...
    def _validate_opposite_arm(
        ...
        *,
        ...
        arm = opposite_arm(self.calibration_arm)
        tolerance = (
            self.config.opposite_arm_hold_tolerance_rad
            if tolerance_rad is None
            else tolerance_rad
        )
        error = float(np.max(np.abs(sample.arm_q(arm) - self._opposite_monitor_q)))
        if error > tolerance:
            context = " moved during ownership transition" if transition else " drifted"
            raise ValueError(
                f"held {arm} arm{context} by {error:.4f}rad; limit is "
                f"{tolerance:.4f}rad"
            )
    ...
    def _compose_command(self, calibration_q: np.ndarray) -> np.ndarray:
        if self.calibration_arm == "left":
            return dual_arm_vector(calibration_q, self._opposite_hold_q)
        return dual_arm_vector(self._opposite_hold_q, calibration_q)
