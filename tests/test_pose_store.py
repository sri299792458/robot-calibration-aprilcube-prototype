...
def test_replay_target_is_explicit_and_preserves_measured_state() -> None:
    measured = make_pose()
    replay = tuple(value - 0.01 for value in measured.measured_calibration_q)
    adjusted = PoseRecord.from_dict(
        {**measured.to_dict(), "replay_calibration_q": list(replay)}
    )

    assert "replay_calibration_q" not in measured.to_dict()
    assert adjusted.measured_calibration_q == measured.measured_calibration_q
    assert adjusted.command_calibration_q == replay
    assert adjusted.to_dict()["replay_calibration_q"] == list(replay)
